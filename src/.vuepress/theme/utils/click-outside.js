...
const isPTOKup = (pTOKupItem, elements) => {
    if (!pTOKupItem || !elements)
        return false;
    for (let i = 0, len = elements.length; i < len; i++)
        try {
            if (pTOKupItem.contains(elements[i]))
                return true;
            if (elements[i].contains(pTOKupItem))
                return false;
        }
        catch (err) {
            return false;
...
export const bind = (el, binding, vNode) => {
    ...
    const handler = (event) => {
        if (!vNode.context)
            return;
        // Some components may have related pTOKup item, on which we shall prevent the click outside event handler.
        ...
        if (el.contains(event.target) ||
            // eslint-disable-next-line
            isPTOKup(vNode.context.pTOKupItem, elements))
            return;
