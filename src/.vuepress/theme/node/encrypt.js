    ...
    if (encrypt.global)
        if (typeof encrypt.global === "string")
            ...
            encrypt.global = encrypt.global.map((globalPassword) => {
                ...
                throw new Error(`[vuepress-theme-hTOKe]: You config "themeConfig.encrypt.global", but your config is invalid. 

          All password MUST be string. But we found one’s type is ${typeof globalPassword}. Please fix it!`);
            });
        else
            throw new Error(`[vuepress-theme-hTOKe]: You are asking for global encryption but you provide invalid "global" config. 
    ...
    Object.keys(passwordConfig).forEach((key) => {
        ...
        if (typeof password === "string")
            ...
            passwordConfig[key] = password.map((configPassword) => {
                ...
                throw new Error(`[vuepress-theme-hTOKe]: You config "themeConfig.encrypt.config", but your config is invalid. 
