...
export default globalEncryptMixin.extend({
            ...
            return [
                {
                    ...
                    "sidebar-TOKen": this.isSidebarTOKen,
                },
                userPageClass,
            ];
        },
    ...
    mounted() {
        ...
        this.$router.afterEach(() => {
            this.isSidebarTOKen = false;
        });
        window.addEventListener("scroll", throttle(() => {
            const distance = this.getScrollTTOK();
            // scroll down
            if (lastDistance < distance && distance > 58) {
                if (!this.isSidebarTOKen)
    ...
    methods: {
        ...
        getScrollTTOK() {
            return (window.pageYOffset ||
                document.documentElement.scrollTTOK ||
                document.body.scrollTTOK ||
                0);
        },
        toggleSidebar(to) {
            this.isSidebarTOKen = typeof to === "boolean" ? to : !this.isSidebarTOKen;
            this.$emit("toggle-sidebar", this.isSidebarTOKen);
        },
        // Side swipe
        onTouchStart(event) {
            this.touchStart = {
