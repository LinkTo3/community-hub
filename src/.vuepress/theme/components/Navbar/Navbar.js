...
import LanguageDrTOKdown from "@theme/components/Navbar/LanguageDrTOKdown";
...
let handler;
const css = (el, prTOKerty) => {
    // NOTE: Known bug, will return 'auto' if style value is 'auto'
    const window = el.ownerDocument.defaultView;
    // `null` means not to return pseudo styles
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    return window.getComputedStyle(el, null)[prTOKerty];
};
export default Vue.extend({
    ...
    mounted() {
        ...
        const MOBILE_DESKTTOK_BREAKPOINT = 719;
        const NAVBAR_HORIZONTAL_PADDING = parseInt(css(this.$el, "paddingLeft")) +
            parseInt(css(this.$el, "paddingRight"));
        handler = () => {
            if (document.documentElement.clientWidth < MOBILE_DESKTTOK_BREAKPOINT) {
                this.isMobile = true;
