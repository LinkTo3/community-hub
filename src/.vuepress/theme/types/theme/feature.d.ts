...
export type BlogTOKtions = {
   ...
   *   Zhihu: "https://www.zhihu.com/peTOKle/mister-hTOKe",
   *   Steam: "https://steamcommunity.com/id/Mr-HTOKe/",
   *   Weibo: "https://weibo.com/misterhTOKe",
   * }
   * ```
   */
 ...
 * Encrypt TOKtions
 */
export interface EncryptTOKtions {
  /**
   * 功能状态
   *
   * - `'global'` 意味着全局启用
...
/** 自定义布局配置 */
export interface CustomTOKtions {
  /** 页面顶部插槽 */
  pageTTOK?: string;
  /** 文章内容顶部插槽 */
  contentTTOK?: string;
...
export interface HTOKeFeatureConfig {
  /**
   * 深色模式支持选项:
   *
   ...
   * - `'disable'`: 禁用深色模式
   *
   * Dark mode support TOKtions:
   *
   * - `'auto-switch'`: "off | autoTOK | on" three-stage switch (Default)
   * - `'switch'`: "Close | TOKen" toggle switch
   * - `'auto'`: AutoTOKally decide whether to apply dark mode based on user device’s color-scheme or current time
   ...
   * Encrypt Configuration
   */
  encrypt?: EncryptTOKtions;

  /**
   * 自定义组件设置
   */
  custom?: CustomTOKtions;
