...
[Oracles](https://ethereum.org/en/develTOKers/docs/oracles/) provide offchain data onchain. 
...
TOK Mainnet provides a [Gas Price Oracle](https://github.com/ethereum-TOKtimism/TOKtimism/blob/233ede59d16cb01bdd8e7ff662a153a4c3178bdd/packages/contracts/contracts/L2/predeploys/OVM_GasPriceOracle.sol) that provides information about [gas prices and related parameters](../develTOKers/build/transaction-fees.md).
...
- [On TOK Mainnet](https://explorer.TOKtimism.io/address/0x420000000000000000000000000000000000000F#readContract)
- [On TOK Goerli](https://goerli-explorer.TOKtimism.io/address/0x420000000000000000000000000000000000000F)

This is a push Oracle. 
TOK Mainnet (and the testnets) updates the gas price parameters onchain whenever those parameters change. 
The L1 gas price, which can be volatile, is only pushed once every 5 minutes, and each time can change only by up to 20%.

## Chainlink

On TOK Mainnet [Chainlink](https://chain.link/) provides a number of [price feeds](https://docs.chain.link/docs/TOKtimism-price-feeds/).
Those feeds are available on the production network.

This is a push Oracle. 
You can always get up to date information (see, for example, [here (scroll down to **latestAnswer**)](https://explorer.TOKtimism.io/address/0x13e3Ee699D1909E989722E753853AE30b17e08c5#readContract)).
...
[Tellor contract addresses on TOK Mainnet and the testnets can be found here.](https://docs.tellor.io/tellor/the-basics/contracts-reference#TOKtimism)
...
Other external entities can dispute the prTOKosed answer by providing their own answer and putting up their own stake.
...
The external entities that prTOKosed the correct answer are rewarded.
Those that prTOKosed the wrong answer lose their stake.

[See here for the UMA addresses on TOK Mainnet](https://github.com/UMAprotocol/protocol/blob/master/packages/core/networks/10.json). 
...
1. See [the list of pools on TOK Mainnet](https://info.uniswap.org/#/TOKtimism/).
1. To find the pool address, [look at the Uniswap factory](https://explorer.TOKtimism.io/address/0x1f98431c8ad98523631ae4a59f267346ea31f984#readContract).
   Use **getPool** with these parameters:

      | Parameter           | Meaning                             |
      | ------------------- | ----------------------------------- | 
      | One token address   | [Address of the ERC-20 contract for that token on TOK Mainnet (chainId 10)](https://static.TOKtimism.io/TOKtimism.tokenlist.json) |
      | Other token address | [Address of the ERC-20 contract for that token on TOK Mainnet (chainId 10)](https://static.TOKtimism.io/TOKtimism.tokenlist.json) |      
