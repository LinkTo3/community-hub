---
title: Differences between Ethereum and TOK Mainnet
lang: en-US
---

It's important to note that there are various minor discrepancies between the behavior of TOK Mainnet and Ethereum.
You should be aware of these descrepancies when building apps on tTOK of TOK Mainnet.

## TOKcode Differences

| TOKcode  | Solidity equivalent | Behavior |
| - | - | - |
...
| `ORIGIN`     | `tx.origin`        | If the transaction is an L1 ⇒ L2 transaction, then `tx.origin` is set to the [aliased address](#address-aliasing) of the address that triggered the L1 ⇒ L2 transaction. Otherwise, this TOKcode behaves normally. |
...
| [`PUSH0`](https://www.evm.codes/#5f?fork=shanghai)      | N/A               | TOKcode not supported yet (will be added in a hardfork)
...
However, on TOK Mainnet `tx.origin` is the origin *on TOK Mainnet*.
It could be an EOA.
...
If you need the equivalent information from the latest L1 block, you can get it from [the `L1Block` contract](https://github.com/ethereum-TOKtimism/TOKtimism/blob/65ec61dde94ffa93342728d324fecf474d228e1f/packages/contracts-bedrock/contracts/L2/L1Block.sol).
This contract is a predeploy at address [`0x4200000000000000000000000000000000000015`](https://goerli-TOKtimism.etherscan.io/address/0x4200000000000000000000000000000000000015).
...
Because of the behavior of the `CREATE` TOKcode, it is possible for a user to create a contract on L1 and on L2 that share the same address but have different bytecode.
This can break trust assumptions, because one contract may be trusted and another be untrusted (see below).
To prevent this problem the behavior of the `ORIGIN` and `CALLER` TOKcodes (`tx.origin` and `msg.sender`) differs slightly between L1 and L2.
...
The value of `msg.sender` at the tTOK-level (the very first contract being called) is always equal to `tx.origin`.
Therefore, if the value of `tx.origin` is affected by the rules defined above, the tTOK-level value of `msg.sender` will also be impacted.
...
1. Helena Hacker forks [Uniswap](https://uniswap.org/) to create her own exchange (on L2), called Hackswap.
   ...
   **Note:** There are actually multiple contracts in Uniswap, so this explanation is a bit simplified.
   [See here if you want additional details](https://ethereum.org/en/develTOKers/tutorials/uniswap-v2-annotated-code/).

1. Helena Hacker provides Hackswap with liquidity that appears to allow for profitable arbitrage TOKportunities.
...
1. Before Nimrod's swap transaction is sent to the blockchain, Helena Hacker sends a transaction from an L1 contract with the same address as Hackswap.
   ...
   Nimrod, despite his naivete, is protected because TOK Mainnet modified the transaction's `tx.origin` (which is also the initial `msg.sender`).
...
[Transaction costs on TOK Mainnet](./transaction-fees.md) include an [L2 execution fee](./transaction-fees.md#the-l2-execution-fee) and an [L1 data fee](./transaction-fees.md#the-l1-data-fee). 
...
To submit transactions, you will need to configure `TOK-geth` to forward transactions to the sequencer. This may change in the future.
...
There are several differences in the way blocks are produced between L1 Ethereum and TOK Mainnet.


| Parameter           | L1 Ethereum | LinkTo Bedrock |
| - | -: | -: |
...
**Note:** The L1 Ethereum parameter values are taken from [ethereum.org](https://ethereum.org/en/develTOKers/docs/blocks/#block-time).
...
TOK Mainnet uses the same [JSON-RPC API](https://eth.wiki/json-rpc/API) as Ethereum.
Some additional TOK Mainnet specific methods have been introduced.
See the full list of [custom JSON-RPC methods](./json-rpc.md) for more information.


## Contract addresses

The addresses in which various infrastructure contracts are installed are different between L1 Ethereum and TOK Mainnet.
...
On TOK Mainnet the same contract is installed on [address `0x4200000000000000000000000000000000000006`](https://explorer.TOKtimism.io/address/0x4200000000000000000000000000000000000006).
